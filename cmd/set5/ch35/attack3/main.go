...
/*
The important thing to figure out here is that if g=p-1, then
if b is even, then B will be 1
if b is odd, then B will be p-1

((p-1)(p-1))%p = (p^2)%p - 2p%p +1 = 1
((p-1)(p-1)(p-1))%p = p-1
...
*/
...
func main() {
	...
	err := alice(chanAM, "super secret message here")
	if err != nil {
		log.Fatal(err)
	}
}
...
func mallary(chA, chB chan string) {
	...
	for {
		...
		// passes on to bob: p and p-1.
		fmt.Println("M: sending p and g=p-1 to bob")
		chB <- string(p.Bytes())
		chB <- string(p.Sub(p, big.NewInt(1)).Bytes())
		...
		// Send an altered A to bob
		fmt.Println("M: sending A (where A = 1) to bob")
		chB <- string(big.NewInt(1).Bytes())
		...
		// send an altered B to alice
		fmt.Println("M: sending B (where B = 1) to alice")
		chA <- string(big.NewInt(1).Bytes())

		// Determine the shared secret:
		sha := sha1.Sum(big.NewInt(1).Bytes())
		s := sha[:16]
