...
func mallary(chA, chB chan string) {
	...
	for {
		...
		// passes on to bob: p and 1.
		fmt.Println("M: sending p and g=p to bob")
		chB <- string(p.Bytes())
		chB <- string(p.Bytes())
		...
		fmt.Println("M: sending A (where A = 0) to bob")
		chB <- string(big.NewInt(0).Bytes())
		...
		// from bob's side: B = (p^b)%p = 0; s = A^b = 0 (since we told him
		// A was 0).
		// from alice's side: s = B^a = 0^a = 0
		sha := sha1.Sum(big.NewInt(0).Bytes())
		s := sha[:16]
